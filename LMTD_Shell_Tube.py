...
data = pd.read_excel(r'input.xlsx','Input_S&T')
...
# verifica se mixture oppure no e inserimento frazioni

mixture[wf_position] = data.Selected_Value[37]
fraction[wf_position] = data.Selected_Value[38]
mixture[sf_position] = data.Selected_Value[39]
fraction[sf_position] = data.Selected_Value[40]
...
fluid[wf_position] = data.Selected_Value[35]
[pressure_inlet[wf_position], temperature_inlet[wf_position], quality_inlet[wf_position], density_inlet[wf_position], enthalpy_inlet[wf_position], viscosity_inlet[wf_position], pressure_outlet[wf_position], temperature_outlet[wf_position], quality_outlet[wf_position], density_outlet[wf_position], enthalpy_outlet[wf_position], viscosity_outlet[wf_position]] = FlowBC.InletOutlet(mixture[wf_position], fraction[wf_position], fluid[wf_position], P_loss[wf_position], P_inlet = data.Selected_Value[2], T_inlet = data.Selected_Value[3], T_outlet = data.Selected_Value[4], Q_inlet = data.Selected_Value[5], Q_outlet = data.Selected_Value[6])
#
...
# print(M[wf_position], enthalpy_inlet[wf_position], enthalpy_outlet[wf_position], enthalpy_inlet[sf_position], enthalpy_outlet[sf_position])
...
Delta_T1 = abs(temperature_inlet[wf_position] - temperature_outlet[sf_position])
Delta_T2 = abs(temperature_outlet[wf_position] - temperature_inlet[sf_position])
...
### QUESTA FORMULA VA VERIFICATA PER SCAMBIATORI A PIU' PASSAGGI ###
...
# Il progettista sceglie se il diametro del tubo per lo shell risptterà gli standard delle tubature
# o se si ha la possibilità di farne produrre uno ad hoc

if data.Selected_Value[34] == 'standard':
    ...
    if D_shell_int < 24*0.0254: # conversione da inch a m
        ...
        # variabile 'data_tube' 
        data_tube = pd.read_excel(r'tubi.xlsx')
...
elif data.Selected_Value[34] == 'custom':
    print ('Selected shell is not standard. \n')

else:
    print ('Undefined D shell selection input. Check input file. \n')
...
if Re_inlet[0] < 10000:
    print("WARNING: basso Reynolds all'ingresso dello scambiatore lato tubi!\n")
if Re_outlet[0] < 10000:
    print("WARNING: basso Reynolds all'uscita dello scambiatore lato tubi!\n")
...
# Questo deve essere un valore compresso tra 0.2 ed 1. Ft_check
decision = 'not accepted'
...
# Calcolo della velocità di attraversamento lato mantello 24
...
if quality_inlet[0] == quality_outlet[0]:
    # alfa_inlet[0] = HT.heat_transfer_coefficient_1ph(mixture[0],fraction[0],temperature_mean,pressure_mean,fluid,Re_mean,j_h,D_eq[0],0,L)
    alfa_inlet[0] = HT.heat_transfer_coefficient_1ph_Tube(mixture[0],fraction[0],temperature_inlet[0],temperature_inlet[1],pressure_inlet[0],viscosity_inlet[0],Re_inlet[0],fluid[0],L,ID)
    alfa_outlet[0] = HT.heat_transfer_coefficient_1ph_Tube(mixture[0],fraction[0],temperature_outlet[0],temperature_outlet[1],pressure_outlet[0],viscosity_outlet[0],Re_outlet[0],fluid[0],L,ID)
    alfa[0] = np.mean( [alfa_inlet[0],alfa_outlet[0]] )
# - Condensazione
elif quality_inlet[0] > quality_outlet[0]:
    if data.Selected_Value[33] == 'horizontal':
        alfa_inlet[0] = HT.heat_exchange_coefficient_condensation_horizontal(mixture[0],fraction[0],pressure_inlet[0],quality_inlet[0],Re_inlet[0],M[0],fluid[0],ID,A_cross[0])
        alfa_outlet[0] = HT.heat_exchange_coefficient_condensation_horizontal(mixture[0],fraction[0],pressure_outlet[0],quality_outlet[0],Re_outlet[0],M[0],fluid[0],ID,A_cross[0])
        alfa[0] = np.mean( [alfa_inlet[0],alfa_outlet[0]] )
    elif data.Selected_Value[33] == 'vertical':
        alfa_inlet[0] = HT.heat_exchange_coefficient_condensation_vertical(mixture[0],fraction[0],pressure_inlet[0],quality_inlet[0],quality_outlet[0],Re_inlet[0],M[0],fluid[0],ID,A_cross[0])
        alfa_outlet[0] = HT.heat_exchange_coefficient_condensation_vertical(mixture[0],fraction[0],pressure_outlet[0],quality_inlet[0],quality_outlet[0],Re_outlet[0],M[0],fluid[0],ID,A_cross[0])
        alfa[0] = np.mean( [alfa_inlet[0],alfa_outlet[0]] )
# - Evaporazione
else:
    if data.Selected_Value[33] == 'horizontal':
        alfa_inlet[0] = HT.heat_exchange_coefficient_evaporation_horizontal(mixture[0],fraction[0],pressure_inlet[0],quality_inlet[0],np.pi*(ID/2)**2,ID,fluid[0],Q/A,M[0]/A_cross[0])
        alfa_outlet[0] = HT.heat_exchange_coefficient_evaporation_horizontal(mixture[0],fraction[0],pressure_outlet[0],quality_outlet[0],np.pi*(ID/2)**2,ID,fluid[0],Q/A,M[0]/A_cross[0])
        alfa[0] = np.mean( [alfa_inlet[0],alfa_outlet[0]] )
    elif data.Selected_Value[33] == 'vertical':
        alfa_inlet[0] = HT.heat_exchange_coefficient_evaporation_vertical(mixture[0],fraction[0],pressure_inlet[0],temperature_inlet[0],quality_inlet[0],Re_inlet[0],viscosity_inlet[0],ID,Q/A,fluid[0])
        alfa_outlet[0] = HT.heat_exchange_coefficient_evaporation_vertical(mixture[0],fraction[0],pressure_outlet[0],temperature_outlet[0],quality_outlet[0],Re_outlet[0],viscosity_outlet[0],ID,Q/A,fluid[0])
        alfa[0] = np.mean( [alfa_inlet[0],alfa_outlet[0]] )    
...
if quality_inlet[1] == quality_outlet[1]:
    alfa_inlet[1] = HT.heat_transfer_coefficient_1ph_Shell(mixture[1],fraction[1],temperature_inlet[1],temperature_outlet[0],pressure_inlet[1],velocity_inlet[1],viscosity_inlet[1],fluid[1],density_inlet[1],D_eq[1],OD,nt,D_shell_int,baffle_cut,B,D_bundle,baffle_hole_d,pitch,tube_disposition)
    alfa_outlet[1] = HT.heat_transfer_coefficient_1ph_Shell(mixture[1],fraction[1],temperature_outlet[1],temperature_inlet[0],pressure_outlet[1],velocity_outlet[1],viscosity_outlet[1],fluid[1],density_outlet[1],D_eq[1],OD,nt,D_shell_int,baffle_cut,B,D_bundle,baffle_hole_d,pitch,tube_disposition)
    alfa[1] = np.mean( [alfa_inlet[1],alfa_outlet[1]] )
# - Condensazione
elif quality_inlet[1] > quality_outlet[1]:
    alfa_inlet[1] = HT.heat_exchange_coefficient_condensation_horizontal(mixture[1],fraction[1],pressure_inlet[1],quality_inlet[1],fluid[1],velocity_inlet[1],T_wall,Re_inlet[1],D_eq[1])
    alfa_outlet[1] = HT.heat_exchange_coefficient_condensation_horizontal(mixture[1],fraction[1],pressure_outlet[1],quality_outlet[1],fluid[1],velocity_outlet[1],T_wall,Re_outlet[1],D_eq[1])
    alfa[1] = np.mean( [alfa_inlet[1],alfa_outlet[1]] )
# - Evaporazione
else:
    alfa_inlet[1] = HT.heat_transfer_coefficient_evaporation_Shell(mixture[1],fraction[1],pressure_inlet[1],quality_inlet[1],fluid[1],pitch,A_cross[1],OD,Q/A)
    alfa_outlet[1] = HT.heat_transfer_coefficient_evaporation_Shell(mixture[1],fraction[1],pressure_outlet[1],quality_outlet[1],fluid[1],pitch,A_cross[1],OD,Q/A)
    alfa[1] = np.mean( [alfa_inlet[1],alfa_outlet[1]] )
...
print('RISULTATI DIMENSIONAMENTO:')
...
print('the number of tubes nt is: ', float(nt))
print('the shell inner diameter D_shell_int is: ', float("{0:.2f}".format(D_shell_int)))
print('the exchange area A is: ', float("{0:.2f}".format(A)), 'm^2')
print('Buffle spacing absolute: ', float("{0:.2f}".format(B)), 'm')
print('the secondary fluid mass flow M is: ', float("{0:.2f}".format(M[sf_position])), 'kg/s')
print('the real exchanged thermal power Q_star is: ', float("{0:.2f}".format(Q_star)), 'W')
print('the tubes outer diameter OD is: ', float("{0:.2f}".format(OD*1000)), 'mm')
print('the tubes inner diameter ID is: ', float("{0:.2f}".format(ID*1000)), 'mm')
print('the mean velocity of the fluid in the tubes is: ', float("{0:.2f}".format(velocity_mean[0])), 'm/s. Please, check if this velocity is acceptable. Some tables are uploaded on the input file')
print('the mean velocity of the fluid in the shell is: ', float("{0:.2f}".format(velocity_mean[1])), 'm/s. Please, check if this velocity is acceptable. Some tables are uploaded on the input file')
output = pd.DataFrame({'Variable': ['Q_initial', 'Uo_star', 'Uo_star%Uo', 'P loss tube', 'P loss shell', 'alfa tube', 'alfa shell', 'nt', 'D_shell_int', 'A', 'Buffle spacing, m', 'M2', 'Q_star', 'OD', 'ID', 'V tubes in', 'V tubes out', 'V shell out', 'V shell out', 'Re tubes inlet', 'Re tubes outlet', 'Re shell inlet', 'Re shell outlet'],
                   'Values': [Q, Uo_star, 100*(Uo_star-Uo)/Uo, delta_P[0], delta_P[1], alfa[0], alfa[1], nt, D_shell_int, A, B, M[sf_position], Q_star, OD*1000, ID*1000, velocity_inlet[0], velocity_outlet[0], velocity_inlet[1], velocity_outlet[1], Re_inlet[0], Re_outlet[0], Re_inlet[1], Re_outlet[1]],
                   'Units': ['W', 'W/(m2*K)', '%', 'Pa', 'Pa', 'W/(m2*K)', 'W/(m2*K)', ' ', 'm', 'm^2', 'm', 'kg/s', 'W', 'mm', 'mm', 'm^s', 'm^s', 'm^s', 'm^s', ' ',' ', ' ',' ']})
# writer = pd.ExcelWriter(r'C:\Users\Utente\OneDrive\Codici\Scambiatori Shell&Tube\output.xlsx', engine='xlsxwriter')
writer = pd.ExcelWriter(r'C:\\Users\\Utente\\Desktop\\output.xlsx', engine='xlsxwriter')
